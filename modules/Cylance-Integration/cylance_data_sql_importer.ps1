...
$APIToken = (Invoke-SqlCmd -ServerInstance $cache:sql_instance -Database $cache:db_name -Query "SELECT apikey FROM template_configs WHERE template_name = 'Cylance'").apikey
...
ForEach ($Device in $DevicesData) {
    ...
    Invoke-SqlCmd -ServerInstance $cache:sql_instance -Database $cache:db_name -Query "INSERT INTO cylance_device_data (
...
Foreach ($Event in $EventsData) {
    ...
    Invoke-Sqlcmd -ServerInstance $cache:sql_instance -Database $cache:db_name -Query "INSERT INTO cylance_event_data (sha256,md5,device_name,date,file_path,event_status,cylance_score,classification,running,ever_run,detected_by,serial_number)
...
Foreach ($ClearedEvent in $ClearedData) {
    ...
    Invoke-Sqlcmd -ServerInstance $cache:sql_instance -Database $cache:db_name -Query "INSERT INTO cylance_cleared_data (sha256,md5,device_name,date_removed,file_path,cylance_score,classification,running,ever_run,detected_by)
...
Foreach ($MemEvent in $MemoryprotectionData) {
    ...
    Invoke-Sqlcmd -ServerInstance $cache:sql_instance -Database $cache:db_name -Query "INSERT INTO cylance_memprotect_data(device_name,serial_number,process_name,added,process_id,type,action,user_name)
